...
function createHaul({
  ...
  return new Promise((resolve, reject) => {
    const stmt = db.prepare(
      'INSERT INTO hauls (name, genre, item_count, duration_days, cost, cover_image, profile_id) VALUES (?, ?, ?, ?, ?, ?, ?)',
    );
    stmt.run(
      name.trim(),
...
function updateHaul(id, { name, genre, itemCount, durationDays, cost, coverImage }) {
  return new Promise((resolve, reject) => {
    const stmt = db.prepare(
      `UPDATE hauls
       SET name = ?, genre = ?, item_count = ?, duration_days = ?, cost = ?, cover_image = COALESCE(?, cover_image)
       WHERE id = ?`,
    );
    stmt.run(
      name.trim(),
...
function buildDashboardData(hauls, sales) {
  ...
  return { activeHauls, expiredHauls, previousHauls, stats };
}
...
app.post('/hauls', upload.single('haulImage'), async (req, res) => {
  ...
  const { haulName, genre, itemCount, durationDays, cost } = req.body;
...
app.post(
  ...
  const { itemName, platform, soldPrice, itemCost, buyerName, sellingAccount } =
    req.body;
...
app.get('/tools/:id/download', async (req, res) => {
  ...
  try {
    ...
    const fullPath = path.join(__dirname, receipt.path);
